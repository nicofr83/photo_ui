...
import { getRender, getThumb } from '../metier/images/image_service.ts';
...
export function registerImagesRoutes(server: FastifyInstance, deps: ImagesRoutesDeps): void {
  const { pool, thumbsRoot, originalsRoot, renderCacheRoot, safeFs } = deps;
  const inFlight = new InFlightRenders(deps.renderConcurrency);
  const serviceDeps = { thumbsRoot, originalsRoot, renderCacheRoot, safeFs, inFlight };
  ...
  server.get('/images/:sha256/thumb', async (request, reply) => {
    ...
    const result = await getThumb(serviceDeps, sha256);
  ...
  server.get('/images/:sha256/render', async (request, reply) => {
    ...
    const result = await getRender(serviceDeps, sha256, photo, edge);
